...
class AurumMarketScraper extends AbstractScraper
{
    ...
    private function quotesFor(float $vendorAdjustment): array
    {
        ...
        return collect($catalog)->map(function (array $product, int $index) use ($vendorAdjustment) {
            $buy = $this->price((float) $product['buy'], $vendorAdjustment, $index);
            $sell = $this->price((float) $product['sell'], $vendorAdjustment, $index + 2);
    ...
    {
        return ((int) now()->format('i') + $seed) % 11 === 0 ? 'out_of_stock' : 'in_stock';
    }
}
